...
def fetch_data(db):
    ...
    justice_vote_case_data = pandas.merge(justice_vote_case_data, votes, on=['case_id'])
    ...
    justice_vote_case_data.dropna(subset=['facts', 'date_argued', 'document_petitioner', 'document_respondent', 'majority'], 
                                  inplace=True)
